function [L,obj,tsvd_rank,iter,LRE] =HTNN_FFT(X,Omega,opts)
...
%      iter         -    iterations
%       LRE           -   Relative error at each step of iteration
% Written by  Wenjin Qin  (qinwenjin2021@163.com)
%

mu = 1e-3;%% mu is beta in the paper
max_mu = 1e8;
tol = 1e-6; 
max_iter = 500;
rho = 1.2;
...
dim = size(X);
p = length(size(X));% the is d in the paper (order)
n = zeros(1,p);
for i = 1:p
    n(i) = size(X,i);
end
...
for iter = 1 : max_iter
    ...
    % update Mbar  %%K^(k+1/2) in the paper
    Mbar = L + E + Y/mu;
    Mbar = X.*Omega + Mbar.*BarOmega;
    
    % update L %%X in the paper
    [L,tnnX,trank] =prox_htnn_F(Mbar-E-Y/mu,1/mu); 
    
    % updata M %%K in the paper
    M = L + E + Y/mu;
